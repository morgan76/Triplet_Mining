...
def embed_beats(A_rep, A_loc, config):
    ...
    #print('Shape A_rep', A_rep.shape, flush=True)
    ...
    #print('A_loc shape',A_loc.shape)
    path_distance = np.sum(np.diff(A_loc, axis=1)**2, axis=0)
    #print('Path distance shape',path_distance.shape)
    sigma = np.median(path_distance)
    path_sim = np.exp(-path_distance / sigma)

    R_path = np.diag(path_sim, k=1) + np.diag(path_sim, k=-1)
    #print('R path shape',R_path.shape)
...
def cluster(evecs, Cnorm, k, in_bound_idxs=None):
    ...
    #n_clusters = k
    #size_max = X.shape[0]//n_clusters
    #KM = KMeansConstrained(n_clusters=n_clusters,size_min=16,size_max=size_max,random_state=0)
...
def do_segmentation(C, M, hier, in_bound_idxs=None):
    ...
    #temp = [(est_idxs[i], est_labels[i]) for i in range(len(est_idxs))]

    #(est_idxs, est_labels) = reindex(temp)
